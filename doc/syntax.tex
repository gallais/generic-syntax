...
Altenkirch and Reus~\citeyear{altenkirch1999monadic} introduced the
...
\]
The operation of renaming free variables in a term then corresponds to
the fixpoint's functoriality, and stability under substitution is given
by the proof that the fixpoint forms a monad.
...
or adjusted (e.g. in the $\lambda$-abstraction's case) by introducing two
combinators. The first one is the pointwise lifting of the function space
silently threading the underlying scope whereas the second one makes explicit
the \emph{adjustment} made to it by a function. The use of $\_\vdash{}\_$\footnote{
...
\ExecuteMetaData[indexed.tex]{parrow}
\end{minipage}\hspace{2em}
\begin{minipage}{0.45\textwidth}
\ExecuteMetaData[indexed.tex]{pextend}
\end{minipage}
%\ExecuteMetaData[indexed.tex]{pforall}
\caption{Combinators to build indexed Sets}
\end{figure}

Using these combinators together with \AF{[\_]} which turns an indexed Set
into a Set by implicitly quantifying over the index, the untyped
$\lambda$-calculus can be represented using the following definitions:

\begin{figure}[h]
\begin{minipage}{0.45\textwidth}
\ExecuteMetaData[motivation.tex]{var}
\end{minipage}\hspace{2em}
\begin{minipage}{0.45\textwidth}
...
altought we will abstain from unfolding them in the future\footnote{That would defeat the
purpose of introducing them in the first place, after all.}, we do so here in the hope
...
A \AF{Thinning} from $m$ to $n$ is a function from \AD{Var} $m$ to \AD{Var} $n$.
It subsumes more structured notions such as the Category of
Weakenings~\cite{altenkirch1995categorical} or Order Preserving
Embeddings~\cite{chapman2009type}. The fact that the representation used
...
them under binders. Unsurprisingly, the \AF{□} combinator turns any
...
\begin{figure}[h]
\begin{minipage}{0.45\textwidth}
\ExecuteMetaData[motivation.tex]{box}
\end{minipage}\hspace{2em}
\begin{minipage}{0.45\textwidth}
\ExecuteMetaData[motivation.tex]{thinnable}
\end{minipage}
