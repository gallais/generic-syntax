...
the underlying scope. The third one \AF{\_⊢\_} makes explicit
...
Last but not least \AF{□} is a comonad: \AF{extract} applies the identity \AF{Thinning}
to its argument, and \AF{duplicate} is obtained by composing the two \AF{Thinning}s
...
recursive substructure indexed by $J$ and \AIC{`∎} to stop, enforcing the
...
a generic program over them. The decoding function \AF{⟦\_⟧} acted on the objects
of $\Set{}^J$, and we will now define the function \AF{fmap} by recursion over a
code \AB{d}. It describes the action of the functor corresponding to \AB{d} over
...
of type \AD{List} \AB{I} \AS{→} \AB{I} \AF{─Scoped} by simply appending the two
...
term it goes through, it makes sense for the algebra to be implemented using \AF{fmap}.
