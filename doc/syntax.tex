...
Nowadays the programmer writing an embedded DSL~\cite{hudak1996building}
...
families of Type Theory~\cite{dybjer1994inductive} for representing their syntax,
programmers can
...
Solutions range from enforcing only well scopedness to ensuring
full type and scope correctness.

We start with primers on scoped and sorted terms (Section~\ref{section:primer-term}),
scope and sort safe programs acting on them (Section~\ref{section:primer-program}),
and programmable descriptions of data types (Section~\ref{section:data}).
...
of scope safe syntaxes with binding (Section~\ref{section:universe}) together
with a generic notion of scope safe semantics for these syntaxes (Section~\ref{section:semantics}).
This gives us the opportunity to write generic implementations of renaming
and substitution (Section~\ref{section:renandsub}) but also elaboration of
a surface language to a core one (Section~\ref{section:letbinding}),
and normalisation by evaluation (Section~\ref{section:nbyeval}. We also explore opportunities
for generic proving by describing a framework to formally describe what
it means for a semantics to be able to simulate another one (Section~\ref{section:simulation}),
or for two semantics to be fusible (Section~\ref{section:fusion}).
...
nowadays classic presentation of a scope safe languages by using inductive
...
we can track scoping information at the type level. As the empty $\Set$ has no
inhabitant, it is a natural representation of the empty scope. The functor
$M(X) = 1 + X$ is used to extend the running scope with an extra variable.

An inductive type can be seen as the fixpoint of an endofunctor on $\Set{}$.
Similarly, an inductive family can be obtained by taking the fixpoint on an
...
We pick as our $J$ the category whose objects are inhabitants of \AD{List} \AB{I}
(\AB{I} is a parameter of the construction meant to represents the set of sorts for
our language) and whose morphisms are thinnings (see Section~\ref{def:thinning}).
This \AD{List} \AB{I} is intended to represent the list
...
current scope is either passed unchanged to subterms (e.g. in the application's case)
or extended (e.g. in the $\lambda$-abstraction's case) by introducing combinators
to build indexed types. All the definitions are polymorphic in the index type
(called \AB{A} in Figure~\ref{figure:indexed}).

The first two combinators \AF{\_⟶\_} and \AF{\_∙×\_} are the pointwise liftings
of the function space and the product type respectively, both silently threading
the underlying scope.

The third combinator \AF{\_⊢\_} makes explicit the \emph{adjustment} made to the index
...
The fourth combinator \AF{κ} is the constant function turning a Set into an indexed Set
...
The inductive family \AD{Var} represents to well scoped and well kinded
...
{\AS{∀} \AB{i} \AB{xs} \AS{→} \AD{Var} \AB{i} (\AB{i} \AIC{:\!:} \AB{xs})} for \AIC{z},
and {\AS{∀} \AB{i} \AB{j} \AB{xs} → \AD{Var} \AB{i} \AB{xs} \AS{→} \AD{Var} \AB{i} (\AB{j} \AIC{:\!:} \AB{xs})} for \AIC{s}.
...
of such traversals. Now that well typedness and scopedness are enforced
statically, all of these traversals have to be implemented
in a type and scope safe manner.

These constraints show up in the types of renaming and substitution which can
...
we can define an abstract concept of semantics for our scope safe language.
...
Semantics are defined in terms of a choice of values \AB{\mathcal{V}} and
...
given a value in \AB{\mathcal{V}} for each variable in scope, turns a term
into a computation \AB{\mathcal{C}}.

\begin{figure}[h]
\ExecuteMetaData[motivation.tex]{sem}
\caption{Fundamental Lemma of Semantics for \AD{Lam}}\label{figure:fdmlamsem}
\end{figure}
...
In both cases we use (\AIC{pack} \AB{s}) (where \AIC{pack} is the constructor
for environments and \AIC{s} the functionn lifting an existing de Bruijn variable
...
recursive substructure indexed by $J$ and \AIC{`∎} to stop with a particular
index value.
...
\caption{Action on Morphisms of the Functor corresponding to a \AF{Descr}iption}
\end{figure}
...
termination checker to see that \AF{fold} is a total function. Sized types
are outside the scope of this paper; describing them in details would be a
distraction.

The CDMM approach therefore allows us to generically define iteration principles
for all data types that can be described. This are exactly the features we desire
...
\AS{Set} anymore but rather on {\AB{I} \AF{─Scoped}}. We now think of the index
...
represent data and index constraints respectively. The \AIC{`X} constructor
is now augmented with an addtional {\AD{List} \AB{I}} argument that describes
the new binders that are brought into scope at this recursive position.
...
newly-bound variables in \AB{Δ}) in an ambiant scope \AB{Γ} as {\AB{X} \AB{Δ} \AB{j} \AB{Γ}}.
...
The astute reader may have noticed that \AF{⟦\_⟧} is uniform in $X$ and $Γ$; however
...
to the original constructors in Figure~\ref{scoped-untyped}: \AIC{`V}
for \AIC{V} the variable constructor, \AIC{`A} for \AIC{A} the
application one and \AIC{`L} for \AIC{L} the $\lambda$-abstraction.
This means that the end user can saemlessly write pattern-matching programs on
...
\item{\ARF{alg}} We should have an algebra turning
      ...
      by the Kripke typed defined below) into Computations
\end{itemize}
...
We can now define the type of the fundamental lemma (called \AF{sem}) as
...
evaluating it in an extended context and feeding it dummy values corresponding to
...
Substitution can be defined in a similar manner with \AD{Tm} as both
...
one is trivial. The algebra can once more be defined by using \AF{fmap}
...
families which have \AR{VarLike} values (\AF{vl^{Var}} and \AF{vl^{Tm}}
...
For any \AR{VarLike} \AB{𝓥}, we can define \AF{fresh^{r}} of
type {(\AB{Γ} \AR{─Env}) \AB{𝓥} (\AB{Δ} \AF{++} \AB{Γ})} and \AF{fresh^{l}} of
...
computations are both let-free terms. The algebra of the semantics can
...
We haven't used the \AIC{⊥} constructor so \emph{if} the evaluation terminates
...
\AF{[0, 1]} and \AF{01↺} which describe respectively a finite list containing
...
semantics can simulate another one. Given a relation \AB{𝓡^𝓥} connecting values
in \AB{𝓥_1} and \AB{𝓥_2}, and a relation \AB{𝓡^𝓒} connecting computations in
...
The notion of fusion is defined for a triple of \AR{Sem}antics; each \AB{𝓢_i}
being defined for values in \AB{𝓥_i} and computations in \AB{𝓒_i}. The
fundamental lemma associated to such a set of constraints will state that
running \AB{𝓢_2} after \AB{𝓢_1} is equivalent to running \AB{𝓢_3} only.

The definition of fusion is parametrised by three relations: \AB{𝓡^E} relates
triples of environments of values in {(\Gamma \AR{─Env}) \AB{𝓥_1} \Delta},
{(\Delta \AR{─Env}) \AB{𝓥_2} \Theta} and {(\Gamma \AR{─Env}) \AB{𝓥_3} \Theta}
respectively; \AB{𝓡^𝓥} relates pairs of values\AB{𝓥_2} and \AB{𝓥_3};
and \AB{𝓡^𝓒}, our notion of equivalence for evaluation results, relates pairs
of computation in \AB{𝓒_2} and \AB{𝓒_3}.

% \ExecuteMetaData[Generic/Fusion.tex]{fusiontype}

The first obstacle we face is the formal definition of ``running \AB{𝓢_2}
after \AB{𝓢_1}'': for this statement to make sense, the result of running
\AB{𝓢_1} ought to be a term. Or rather, we ought to be able to extract a
term from a \AB{𝓒_1}. Hence the first constraint: the existence of a \ARF{quote_1}
...
Then, we have to think about what happens when going under a binder: \AB{𝓢_1}
will produce a \AF{Kripke} function space but we'd like to have a syntactic
value. Provided that \AB{𝓥_1} is \AR{VarLike}, we can make use of \AF{reify}
...
\AB{\rho_1} in {(\Gamma \AR{─Env}) \AB{𝓥_1} \Delta}, \AB{\rho_2} in
{(\Delta \AR{─Env}) \AB{𝓥_2} \Theta}, and \AB{\rho_3} in {(\Gamma \AR{─Env}) \AB{𝓥_3} \Theta}
are related by \AB{𝓡^E} and we are given a thinning \AB{\sigma} from \Theta to \Omega
...
Remembering that \AF{\_>>\_} is used in the definition of \AF{body}~\ref{def:body} to
...
constraint stating that \AF{\_>>\_} is compatible with \AB{𝓡^E}. We demand
as an extra precondition that the values \AB{\rho_2} and \AB{\rho_3} are extended
with are related according to \AB{𝓡^𝓥}. Lastly, for all the types to match up,
\AB{\rho_1} has to be extended with dummy variables.
...
of the terms' constructors. When evaluating a variable, on the one hand \AB{𝓢_1}
will lookup its meaning in the evaluation environment, turn the resulting value into
a computation which will get quoted and then the result will be evaluated with \AB{𝓢_2}.
Provided that all three evaluation environments are related by \AB{𝓡^E} this should
be equivalent to looking up the value in \AB{𝓢_3}'s environment and turning it into a
computation. Hence the constraint \ARF{var^R}:

\ExecuteMetaData[Generic/Fusion.tex]{fusionvar}

The case of the alegbra follows a similar idea albeit being more complex:
a term gets evaluated using \AB{𝓢_1} and in order to be able to run \AB{𝓢_2}
afterwards we need to recover a piece of syntax. This is possible if the
\AF{Kripke} functional spaces are reified by being fed dummy \AB{𝓥_1} arguments
(which can be manufactured thanks to the \ARF{vl^𝓥_1} we mentioned before) and
then quoted. Provided that the result of running \AB{𝓢_2} on that term is
related via {\AF{Zip} \AB{d} (\AF{Kripke^R} \AB{𝓡^𝓥} \AB{𝓡^𝓒}} to the result
of running \AB{𝓢_3} on the original term, the \ARF{alg^R} constraint states
...
computations: the composition of \AB{𝓢_1} and \AB{𝓢_2} on one hand and
\AB{𝓢_3} on the other. This lemma is once again proven mutually with its
counterpart for \AR{Sem}'s \AF{body}'s action on \AR{Scope}s and the \AD{Size}
...
trivial: the first \AIC{refl} witnesses the fact that both definitions
