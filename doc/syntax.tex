...
as values and \AD{State \mathbb{N} String} as computations. We use a
...
the definitions of \AF{th^{Wrap}} and \AF{map^{Wrap}}.
...
and a recursive substructure (i.e. the tail of the list).\todo{fin}
...
\end{minipage}
\begin{minipage}{0.45\textwidth}
  \ExecuteMetaData[Generic/Data.tex]{finD}
\end{minipage}
...
language. We represent it as a bi-sorted language using a notion of \AD{Phase} to
distinguish between terms for which we will be able to \AIC{Infer} the type and
the ones for which we will have to \AIC{Check} a type candidate.
...
\end{minipage}
\begin{minipage}{0.45\textwidth}
  \ExecuteMetaData[Generic/Examples/TypeChecking.tex]{phase}
\end{minipage}
...
Keuchel, Weirich, and Schrijvers' Needle~\cite{needleandknot} is a code
