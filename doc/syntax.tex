...
structure. We show with \AF{01⋯} the infinite list that to
correspond to the cyclic example \AF{01↺} given above. The definition
...
triples of environments of values in {(\AB{\Gamma} \AR{─Env}) \AB{𝓥_1} \AB{\Delta}},
{(\AB{\Delta} \AR{─Env}) \AB{𝓥_2} \AB{\Theta}} and {(\AB{\Gamma} \AR{─Env}) \AB{𝓥_3} \AB{\Theta}}
...
\AB{\rho_1} in {(\AB{\Gamma} \AR{─Env}) \AB{𝓥_1} \AB{\Delta}}, \AB{\rho_2} in
{(\AB{\Delta} \AR{─Env}) \AB{𝓥_2} \AB{\Theta}}, and \AB{\rho_3} in {(\AB{\Gamma} \AR{─Env}) \AB{𝓥_3} \AB{\Theta}}
are related by \AB{𝓡^E} and we are given a thinning \AB{\sigma} from \AB{\Theta} to \AB{\Omega}
...
\AF{Kripke} functional spaces are reified by being fed placeholder \AB{𝓥_1} arguments
