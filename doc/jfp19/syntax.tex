...
That is, when defining an environment \AB{\rho}, we may either make use
of the constructor \AF{pack}, packaging a function \AB{r} as an
environment \(\AB{\rho} = \AF{pack} \AB{r}\), or else define \AB{\rho}
in terms of the underlying function obtained from it by projecting out
the (in this case, unique) \AF{lookup} field, as \(\AF{lookup} \AB{\rho}
= \AB{r}\). Examples of definition in this style are given in
Figure~\ref{fig:thincomb} below, and throughout the rest of the
paper. Where a record type has more than one field, in order to define
an instance of the type, we need to specify each of its fields in such a
way. By analogy with record/object syntax in other languages, Agda
further supports `dot' notation, so that an equivalent definition here
could be expressed as \(\AB{\rho} .\AF{lookup} = \AB{r}\).  
} 
%%%
...
Using these definitions, we can then implement \AF{reify} as in Figure~\ref{fig:genericreify}.
...
\end{minipage}
\caption{Relation Between \AB{I}\AF{─Scoped} Families and Equality Example}
\label{fig:reldef}
\end{figure}
}
