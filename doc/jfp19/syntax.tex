...
\scarequote{illegal states are unrepresentable}, where illegal states
...
to renaming, the monadic \codequote{return} corresponds to the use of variables
(the \AIC{`var} constructor), and the monadic \codequote{bind} corresponds
...
languages, Agda further supports \scarequote{dot} notation, so that an equivalent
...
which grants us monoid laws \literalquote{for free} as per Jeffrey's
observation~\citeyear{jeffrey2011assoc}. We define the following identity, weaken and (generalised) transitivity combinators for \AF{Thinning}s:
...
\end{minipage}
\begin{minipage}{0.45\textwidth}
  \ExecuteMetaData[Data/Environment.tex]{weaken}
\end{minipage}
...
traversals by using different \scarequote{host languages} for this shallow embedding.

Semantics, specified by a record \semrec{}, are defined in terms
...
\AR{VarLike} in Section~\ref{section:semantics}}.
...
In both cases we use \AF{weaken} defined in Section~\ref{sec:genenvironment} as the definition of the
...
body of the function to which we prepend a \literalquote{λ} binding the name \AB{x}.
...
descriptions formal. Interpretation of descriptions give rise to
...
rest of the description is either the \codequote{stop} token or a pair of an element
...
The resulting
\codequote{bind} operator demands a function which maps variables in \emph{any} sort and
...
If we pre-compose the meaning function \AF{⟦\_⟧} with a notion of \scarequote{de Bruijn scopes}
...
When dealing with the body of a binder, we \scarequote{reify} the \AF{Kripke} function by
...
\codequote{deriving} machinery familiar to Haskell programmers can be implemented
...
binding is convenient. The first author experienced this first hand when tackling the
...
fundamental lemma of simulations arise. The first author's solution
to the POPLMark Reloaded challenge
...
to most model constructions \cite{mitchell1991kripke} states
...
A direct application of these results is the first author's entry
\citeyear{POPLMarkReloaded2019}
to the
...
\end{agdasnippet}
The first obstacle we face is the formal definition of \scarequote{running \AB{𝓢ᴮ}
...
categorical \scarequote{program} for substitution and proofs of its properties, was given
...
the first author's experience solving the POPLMark Reloaded challenge, Agda's pattern
...
\begin{itemize}
\item Our example of elaborating an enriched language to a core
...
\item Our universe only includes syntaxes that allow unrestricted
  ...
  \cite{DBLP:conf/tlca/BentonBPH93,barber96dual},
  or that annotate variables with usage information
  \cite{BrunelGMZ14,GhicaS14,PetricekOM14,context-constrained}, or arrange variables into
...
\item An extension of Dagand and McBride's theory of
  ...
  type inference.
\item The first author's work on the POPLMark Reloaded challenge highlights a need
  ...
  substitution so should the closure).  Similarly, the \scarequote{evaluation
