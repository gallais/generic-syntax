...
\begin{figure}[h]
  ...
  \caption{Scope and Kind Aware de Bruijn Indices}
\end{figure}
...
\subsection{Relation Transformers}

In our exploration of generic proofs about the behaviour of various \AR{Semantics},
we are going to need to lift relations on values and computations to relations on
\AF{Kripke} function spaces or on \AB{d}-shaped terms whose subterms have been
evaluated already. We introduce these two key relation transformers here.
...
to related outputs. We use the relation transformer \AF{All} which lifts a relation
on values to one on environments in a pointwise manner.
