...
reader may have noticed that we use a seemingly out-ot-scope variable
...
be familiar enough with ML-style polymorphic types that this will
...
context. Concretely: we would rather use the rule \AB{appᵢ} than
\AB{appₑ} as the inference rule for application in STLC.
...
or extended for subterms is critical to our whole approach to syntax with binding,
...
scope when defining such families, as for example in Figure~\ref{fig:Var}. 
...
\revc{
%%% 2019-11-12
\paragraph{Record syntax in Agda}
As with (all) other record structures defined in this paper, we are able
to profit from Agda's \emph{copattern} syntax, as introduced in
...
{\ARF{lookup}~\AB{\(\rho\)}~=~\AB{r}}. Examples of definition in this
style are given in Figure~\ref{fig:baseenv} below, and throughout the
rest of the paper. A value of a record type with more than one field
requires each of its fields to be given, either by a named constructor
(or else Agda's default \AK{record} syntax), or in copattern
style. By analogy with record/object syntax in other languages, Agda
further supports `dot' notation, so that an equivalent definition here
could be expressed as {\AB{\(\rho\)}~.\ARF{lookup}~=~\AB{r}}.
%%%
}

We can readily define some basic building blocks for environments.
The empty environment (\AF{ε}) is implemented by remarking that there
can be no variable of type {(\AD{Var} \AB{σ} \AIC{[]})} and to correspondingly
dismiss the case with the impossible pattern \AS{()}. The function \AF{\_∙\_}
extends an existing environment with a new value.
...
\end{minipage}
\caption{Combinators to Build Environments}\label{fig:baseenv}
\end{figure}
...
\revc{In the following paragraphs, we interleave the definition of the record
...
\semrec{} framework. The family \AB{𝓥} of values is respectively the family
...
\end{minipage}
\caption{Renaming and Substitution as Instances of \semrec{}\label{fig:SemInstances}}
\end{figure}
...
We also include the definition of a basic printer relying on a name supply to
highlight the fact that computations can very well be effectful. \revc{The ability
to generate fresh names is given to us by a monad that here we decide to call \AF{Fresh}.
Concretely, \AF{Fresh} is implemented as an instance of the State monad where
...
\revc{The wrapper \AR{Wrap} does not depend on the scope \AB{Γ} so it is
...
running \AF{print} on the term \AF{apply} (first introduced in
...
The first component of the definition of CDMM's universe (Figure~\ref{figure:desc}) is an inductive type of
...
our hands on the types' eliminators by instantiating the generic \AF{fold}.
...
Our universe of scope safe and well kinded syntaxes (defined in Figures~\ref{fig:Desc},~\ref{figure:syntaxmeaning})  follows the same principle
...
\begin{figure}[h]
  ...
  \caption{Description for the Untyped λ-calculus\label{fig:descUTLC}}
\end{figure}
...
\end{minipage}
  \caption{Description for the intrinsically typed STLC\label{fig:descSTLC}}
\end{figure}
...
\end{minipage}
\caption{Respective Pattern Synonyms for \AF{UTLC} and \AF{STLC}. \label{fig:patsLC}}
\end{figure}
...
\caption{A Generic Notion of Semantics\label{defn:Semantics}}
\end{figure}
...
\caption{Substructures as either Computations or Kripke Function Spaces\label{defn:Kripke}}
\end{figure}
...
\caption{Statement of the Fundamental Lemma of \semrec{}\label{defn:semantics}}
\end{figure}
...
semantics for the special cases of closed terms in Figure~\ref{fig:evalclosed}.
...
\end{minipage}
\caption{Generic Renaming and Substitution for All Scope Safe Syntaxes with Binding\label{defn:Ren-Sub}}
\end{figure}
...
which have \AR{VarLike} values, i.e.  values which are \AF{Thinnable} and
...
The first example of two environment being related is the trivial proof \AF{reflᴿ}
that for any environment \AB{ρ}, {(\AR{All} \AF{Eqᴿ} \AB{Γ} \AB{ρ} \AB{ρ})}.
The combinators we introduced to build environments (\AF{ε}, \AF{\_∙\_}, etc.)
have natural relational counterparts. We reuse the same names for them, simply
appending an \AF{ᴿ} suffix.
...
\revb{
A constraint mentioning both relation transformers appears naturally when we want to
say that a semantics can simulate another one. For instance, renaming is simulated
by substitution: we simply have to restrict ourselves to environments mapping variables
to terms which happen to be variables.
...
substitution are extensional \revc{i.e. that given environments equal in
...
POPLMark Reloaded challenge~\citeyear{poplmarkreloaded} where \AF{rensub} was actually needed.
