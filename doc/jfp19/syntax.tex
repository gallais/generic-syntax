...
the constructor \AF{pack}, packaging a function \AB{r} as an environment
{\AB{\(\rho\)}~=~\AF{pack} \AB{r}}, or else define \AB{\(\rho\)} in
terms of the underlying function obtained from it by projecting out the
(in this case, unique) \AF{lookup} field, as {\AF{lookup}
  \AB{\(\rho\)}~=~\AB{r}}. Examples of definition in this style are
given in Figure~\ref{fig:thincomb} below, and throughout the rest of the
paper. A value of a record type with more than one field requires each
of its fields to be given, either by a named constructor (or else Agda's
default \AgdaKeyword{record} syntax), or in copattern style. By analogy
with record/object syntax in other languages, Agda further supports
`dot' notation, so that an equivalent definition here could be expressed
as {\AB{\(\rho\)}~.\AF{lookup}~=~\AB{r}}.
%%%
}
