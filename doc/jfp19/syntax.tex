...
We typically call \AB{Γ}-environment an environment that associates values
to each variable in \AB{Γ}. This informs our notation choice: we write
...
and typed in \AB{Δ} to every entry in \AB{Γ}. Formally, we have the following
...
We can readily define some basic building blocks for environments in
Figure~\ref{fig:baseenv}. The empty environment (\AF{ε}) is implemented
...
constraints on two ({\AF{Type} \AF{─Scoped}}) families \AB{𝓥} and \AB{𝓒} is
...
the building blocks introduced in Figure~\ref{fig:baseenv}.
It takes a context extension from \AB{Δ} to \AB{Θ} in the
...
thinning embedding \AB{Γ} into {(\AB{σ} \AIC{∷} \AB{Γ}}.
...
\paragraph{Un(i)typed λ-calculus} For the untyped case, the lack of
