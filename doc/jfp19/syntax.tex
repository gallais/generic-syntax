...
We can obtain \AF{Names} by reusing the standard library's \AF{All}, a predicate
transformer making sure a predicate holds of all the element of a list. The
definition of \AF{WithNames} is analogous to \AF{Pieces} in the previous section:
we expect \AF{Names} for the newly bound variables. Terms in the raw syntax are
leveraging these definitions: they are either a variable i.e. a \AD{String}
potentially accompanied by some extra information \AB{E} (typically a position in
...
Various things can go wrong during scope checking: evidently a name can
...
two cases. The scope checker will return a computation in the monad \AF{M}
thus allowing us to fail and return an error, the string that caused the
failure and the extra data of type \AB{E} that accompanied it.
...
If the ambient scope is empty then we can only return \AIC{OutOfScope} error.
...
an applicative and by design our terms are traversable~\cite{bird_paterson_1999}.
...
appears free in the body of the let binding it.
...
the tree with accurate count information precisely recording whether let-bound
variables are used \AIC{zero}, \AIC{one}, or \AIC{many} times. This annotation
process can be described as a variation on the \AF{Let} syntax extension of
Section~\ref{section:letbinding}, producing a {(\AB{d} \AF{`+} \AF{CLet})} term
from a {(\AB{d} \AF{`+} \AF{Let})} one.
...
\end{minipage}
\caption{Counters and Counted Lets}
\end{figure}
