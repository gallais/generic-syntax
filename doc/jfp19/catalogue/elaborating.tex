...
\revc{
We can then explain what it means for an elaboration process to produce a
term of the ({\AD{Type} \AF{─Scoped}}) family \AB{T}. Provided a type
\AB{σ} and a context of modes \AB{ms}, for any typing \AB{Γ} of this context
of mode, we should get a value of type {(\AB{T} \AB{σ} \AF{⌞} \AB{Γ} \AF{⌟})}.
}
...
\revc{
We purposefully overload the constructor of the \AD{isArrow} family by calling
...
then elaborate the body \AB{b} of the lambda in a context extended
(using \AF{bind}) with one \AIC{Infer} variable assigned an inference
function thanks to the auxiliary function \AF{var₀}. From this we get
...
\begin{figure}[h]
  ...
  \caption{\AF{Elaborate}, the elaboration semantics}
\end{figure}
