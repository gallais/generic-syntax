...
\begin{minipage}{0.40\textwidth}
 \ExecuteMetaData[Stdlib.tex]{product}
\end{minipage}\hfill
...
\ExecuteMetaData[Data/Environment.tex]{comonad}
\end{minipage}\hfill
\begin{minipage}{0.45\textwidth}
\ExecuteMetaData[Data/Environment.tex]{thinnable}
\ExecuteMetaData[Data/Environment.tex]{freeth}
\end{minipage}
...
are defined in Figure~\ref{figure:lamsem} in terms of a choice of values \AB{𝓥} and
computations \AB{𝓒}. Realisation of a semantics will produce a
computation in \AB{𝓒} for every term whose variables are assigned
values in \AB{𝓥} as demonstrated in Figure~\ref{figure:fdmlamsem}.
A semantics must satisfy constraints on the notions of
values \AB{𝓥} and computations \AB{𝓒} at hand. First of all,
...
(\AF{□} (\AB{𝓥} \AB{$\sigma$} \AF{→} \AB{𝓒} \AB{$\tau$})).
...
made earlier, namely that any given {\AR{Sem} \AB{𝓥}
  \AB{𝓒}} induces a function which, given a value in
\AB{𝓥} for each variable in scope, transforms a  \AD{Lam} term into a
computation \AB{𝓒}.

\begin{figure}[h]
\ExecuteMetaData[motivation.tex]{sem}
\caption{Fundamental Lemma of Semantics for \AD{Lam}, relative to a given \AR{Sem}\;\AB{𝓥}\;\AB{𝓒}}\label{figure:fdmlamsem}
\end{figure}
...
We can then combine \AF{⟦\_⟧ᴿ} and \AF{Kripke\textsuperscript{R}} to express the idea
...
\ARF{var} and \ARF{th\textsuperscript{𝓥}}, this constitutes the whole \AF{Sim}ulation
constraint:
...
of the terms' constructors. When evaluating a variable, on the one hand \AB{𝓢ᴬ}
will look up its meaning in the evaluation environment, turn the resulting value into
a computation which will get quoted and then the result will be evaluated with \AB{𝓢ᴮ}.
...
of running \AB{𝓢ᴬᴮ} on the original term, the \ARF{alg\textsuperscript{R}} constraint states
