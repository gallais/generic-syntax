...
made earlier, namely that any given {\semrec{} \AB{𝓥}
...
The values stored in the environment will attach \AD{Type} information for
bound variables whose \AD{Mode} is \AIC{Infer}, guaranteeing no variable ever
uses another mode. In contrast, the generated computations will, depending
on the mode, either take a type candidate and \AIC{Check} it is valid or
\AIC{Infer} a type for their argument. These computations are always potentially
failing so we use the \AD{Maybe} monad.
