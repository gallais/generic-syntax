...
\uncover<8->{\begin{problem}
  \begin{itemize}
    \item  Write a program transformation from \AD{S} to \AD{T} which elaborates away
      \textrm{let..in..}
    \item Prove this program transformation is ``sensible''.
  \end{itemize}
\end{problem}}
...
\begin{frame}{Codes for Syntaxes}
  ...
  \begin{enumerate}
    ...
    \item \highlight<3>{\text{Have recursive substructures}}
    \item \highlight<4>{\text{Constraint the shape of the indices}}
  \end{enumerate}
  ...
  \begin{align*}
    \only<1>{
      & \AD{D} : \AD{Set} \\
      & \ensuremath{\llbracket\cdot\rrbracket}
         : \AD{D}
    ...
    \only<2>{
      & \AIC{\ensuremath{\sigma}}
         : (\AB{A} : \AD{Set})
         \rightarrow (\AB{A} \rightarrow \AD{D})
         \rightarrow \AD{D} \\
    ...
    \only<3>{
      & \AIC{X}
         ...
         \rightarrow \AD{D}
         \rightarrow \AD{D} \\
      & \sem{\AIC{X}~\AB{\AB{j}~\ensuremath{\Delta}}~\AB{d}}
            {\AB{X}~\AB{i}~\AB{\ensuremath{\Gamma}}}
         = X~\AB{j}~(\AB{\ensuremath{\Delta +\!\!+ \Gamma}})
           ~\AD{\ensuremath{\times}}~
           \sem{\AB{d}}
               {\AB{X}~\AB{i}~\AB{\ensuremath{\Gamma}}}
    }
