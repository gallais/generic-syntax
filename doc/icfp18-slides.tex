  ...
  \begin{align*}
    ...
    \AF{STLC} &= \highlight<4>{\AIC{`\ensuremath{\sigma}}~\AD{`STLC}}~\AF{\$}~\lambda~\AK{where}\\
              & (\AIC{`app}~\AB{\ensuremath{\sigma}}~\AB{\ensuremath{\tau}})
                \rightarrow \highlight<5>{\AIC{`X}~\AIC{[]}~(\AB{\ensuremath{\sigma}}
                ~\AIC{\ensuremath{\Rightarrow}}~ \AB{\ensuremath{\tau}})}~
                (\highlight<6>{\AIC{`X}~\AIC{[]}~\AB{\ensuremath{\sigma}}}~
                (\AIC{`\ensuremath{\blacksquare}}~\highlight<7>{\AB{\ensuremath{\tau}}})) \\
              & (\AIC{`lam}~\AB{\ensuremath{\sigma}}~\AB{\ensuremath{\tau}})
                \rightarrow \highlight<8>{\AIC{`X}~(\AB{\ensuremath{\sigma}} ~\AIC{::}~ \AIC{[]})~\AB{\ensuremath{\tau}}}~ (\AIC{`\ensuremath{\blacksquare}}~\highlight<9>{(\AB{\ensuremath{\sigma}} \AIC{\ensuremath{\Rightarrow}} \AB{\ensuremath{\tau}})}) \\
  \end{align*}
\end{frame}
...
\begin{frame}
  ...
  \begin{align*}
    \AF{sem} :~ & \ensuremath{\forall \lbrace \Gamma~\Delta \rbrace} \rightarrow
