\begin{code}
module generic-syntax-examples where
...
open import indexed
open import var as V hiding (_<$>_)
open import environment hiding (refl ; _<$>_)
open import generic-syntax hiding (⊥)
...
module HuttonsRazor where
 ...
 Eval^Hutt : Sem HuttRaz (λ _ _ → ⊥) (λ _ _ → ℕ)
 Eval^Hutt = record
   { th^𝓥  = ⊥-elim
   ; var   = ⊥-elim
   ; alg   = case proj₁ (λ { (m , n , _) → m + n }) }

module StatePrimitives where
